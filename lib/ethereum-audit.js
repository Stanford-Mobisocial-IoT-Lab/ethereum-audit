...
module.exports = class EthAudit {
    ...
    async unlockEthAccount() {
        ...
        } else if (this._config.PASSPHRASE === '' || this._config.PASSPHRASE === undefined) {
    ...
    async retrieveEthAccounts() {
        try {
            ...
            logger('retrieveEthAccounts(1): ' + util.inspect(response));
            return response;
        } catch (error) {
    ...
    async getAuditKey(reqbody) {
        if (reqbody.index === '' || reqbody.index === undefined) {
            ...
            try {
                ...
                throw Error('Failed to get audit key by index');
            }
        }
    }
    ...
    _insertAuditData(contract, key, data) {
        return new Promise((resolve, reject) => {
            if (this._config.COINBASE_ACCOUNT === '' || this._config.COINBASE_ACCOUNT === undefined) {
            ...
            } else if (this._config.GAS === '' || this._config.GAS === undefined) {
                reject(Error('GAS is not defined.'));
            } else if (this._config.GAS_PRICE === '' || this._config.GAS_PRICE === undefined) {
                ...
                contract.methods.insertAuditData(key, data).send({
                    ...
                    let response = {
                        msg: 'Transaction is sent.',
                        hash: hash
                    };
                    logger('_insertAuditData(3): ' + util.inspect(response));
                }).on('receipt', (receipt) => {
                    let response = {
                        msg: 'Transaction is received and written.',
    ...
    async _getAuditDataByKey(contract, key) {
        try {
            ...
            let response = { result: result };
            logger('_getAuditDataByKey(3): ' + util.inspect(response));
            return response;
        } catch (error) {
    ...
    async _getAuditDataCount(contract) {
        try {
            ...
            let response = { result: result };
            logger('_getAuditDataCount(1): ' + util.inspect(response));
            return response;
        } catch (error) {
    ...
    async _getAuditKey(contract, index) {
        try {
            ...
            logger('_getAuditKey(3): ' + util.inspect(response));
            return response;
        } catch (error) {
    ...
    async _checkIsOwner(contract, addr) {
        try {
            ...
            let response = { result: result };
            logger('_checkIsOwner(3): ' + util.inspect(response));
            return response;
        } catch (error) {
    ...
    async _getAuditDataAll(contract) {
        try {
            let result = [];
            let dataCount = parseInt(await contract.methods.getAuditDataCount().call());
            ...
            let response = { result: result };
            logger('_getAuditDataAll(3): ' + util.inspect(response));
            return response;
        } catch (error) {
    ...
    connectAndAccessContract() {
        return new Promise((resolve, reject) => {
            ...
            } else if (this._config.CONTRACT_ADDR === '' || this._config.CONTRACT_ADDR === undefined) {
